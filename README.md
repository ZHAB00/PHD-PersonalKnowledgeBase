...
默认向量化使用 Ollama 的 `qwen3-embedding:4b`（2560 维）。首次使用前先安装并启动 Ollama，再拉取模型：

```powershell
ollama pull qwen3-embedding:4b
```

如果不想运行 Ollama，可在设置页把向量模型切换为本地 ONNX `BAAI/bge-small-zh-v1.5`（512 维）。切换向量模型后，已有索引的维度可能不匹配，需要重建或重新上传对应知识库。
...
1. 在设置页配置并测试对话模型、向量模型、联网搜索、Neo4j
2. 在知识库页上传文档，等待分块、向量化、图谱抽取完成
...
默认 Ollama 是 2560 维，本地 ONNX 是 512 维。切换 provider 后，需要重建对应知识库的索引。
