...
> 版本：对应当前仓库代码
> 应用版本：桌面安装包 0.1.0 / FastAPI 服务 v2.0.0
> 定位：面向个人的本地知识库问答系统，前后端一体，可选桌面窗口、浏览器访问
...
- 可以源码运行，也可以打包成 Windows 桌面安装包，个人数据默认放在用户目录下。
...
```mermaid
flowchart LR
    UI[桌面窗口 / 浏览器] --> API[FastAPI HTTP / SSE]
...
| Agent | LangChain 1.0+ `create_agent` 与手写流式 Agent 循环 |
...
├── tests/                    测试
├── packaging/                桌面启动、打包、安装脚本
├── landing/                  对外介绍页
├── requirements.txt          依赖
└── README.md / API_DOCS.md / OPERATIONS.md
```
...
- 每轮可以并行调用多个工具，工具结果一次性回填。
- 最大 5 轮，超过后返回“处理超时”提示。
...
1. 短期记忆：SQLite 保存完整历史，送入 LLM 前用字符预算滑动窗口裁剪，预算默认 6000 字符。
...
- 正式打包运行后读取用户 `data/settings.json`。

### 15.2 安装包

`packaging/build.ps1` 流程：

1. 安装 PyInstaller。
2. 可选下载内置向量模型。
3. PyInstaller 打包为 `dist2/PDH-PKG`。
4. 拷贝 Qdrant，清理运行数据。
5. Inno Setup 生成 `output/PDH-PKG-Setup-0.1.0.exe`。

安装器提供：

- 完整安装：程序核心 + 内置向量模型。
- 简洁安装：仅程序核心。
- 自定义安装：固定核心，可选内置模型、可选 Neo4j 组件。

卸载时询问是否删除 `%LOCALAPPDATA%\PDH-PKG` 个人数据。
...
- `README.md`：快速开始与功能介绍。
- `OPERATIONS.md`：运行手册与变更记录。
- `API_DOCS.md`：API 接口说明。
- `启动说明.md`：启动与调试说明。
- `agent评估指标体系.md`：评估指标设计。
- `agent_eval/README.md`：评测工具使用说明。
- `BUG_REPORT.md`：历史关键 Bug 复盘。
